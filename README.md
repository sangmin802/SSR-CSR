...
## 과정정리 일단 이해된부분까지만  
  1. (SSR + SPA인 US일 경우)React앱을 최초 렌더링할 서버버전(SSR)과, 이후 요청에 따라 필요한 웹버전(CSR)으로 두개 build를 할 수 있도록, webpack을 설정한다.(이건 좀 나중에)  
  1. (SSR만 할 경우) 위의 과정 필요 없음.  
  ...
  4. React의 index.js에서 ReactDOM.render -> ReactDOM.hydrate로 변경해준다.
